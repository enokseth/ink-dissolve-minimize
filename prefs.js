...
export default class Prefs extends ExtensionPreferences {
  ...
  gTop.add(easyDriftRow);
    ...
    const durRow = new Adw.ActionRow({ title: 'Duration (ms)' });
    const durAdj = new Gtk.Adjustment({ lower: 100, upper: 2000, step_increment: 50, page_increment: 100 });
    const durScale = new Gtk.Scale({ adjustment: durAdj, digits: 0, hexpand: true, value_pos: Gtk.PositionType.RIGHT });
    durScale.set_value(data.DURATION.get());
    durScale.connect('value-changed', w => data.DURATION.set(Math.round(w.get_value())));
    durRow.add_suffix(durScale);
    g1.add(durRow);
  ...
  g2.add(shaderRow);
    ...
    resetBtn.connect('clicked', () => {
      ...
      if (data.SIMPLE_MODE.get()) {
        ...
        applyPreset('default', data, { durScale, scale, intens });
        easyShrinkScale.set_value(data.SHRINK_MIN.get());
        easyDriftScale.set_value(data.DRIFT_PX.get());
      } else {
  ...
  if (header) header.pack_start(previewBtn);
    ...
    function applyPreset(name, data, widgets) {
      ...
      if (widgets.durScale) widgets.durScale.set_value(cfg.dur);
  ...
  toggleAdvanced(data.SIMPLE_MODE.get());
    if (data.SIMPLE_MODE.get()) {
      applyPreset(data.PRESET.get(), data, { durScale, scale, intens });
    }
